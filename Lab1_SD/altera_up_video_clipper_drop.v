...
parameter WIDTH					= 640; // Image in width in pixels
parameter HEIGHT				= 480; // Image in height in lines
...
parameter ADD_DATA				= 'h0; // Data for added pixels
...
begin
	if (reset)
		present_state <= STATE_0_WAIT_FOR_START;
	else
	...
	STATE_0_WAIT_FOR_START:
		begin
			if (stream_in_startofpacket & stream_in_valid)
				next_state = STATE_1_RUN_CLIPPER;
			else
				next_state = STATE_0_WAIT_FOR_START;
		end
	STATE_1_RUN_CLIPPER:
		begin
			if (increment_counters & full_frame_endofpacket)
				...
				next_state = STATE_2_ADD_MISSING_PART;
			else
				next_state = STATE_1_RUN_CLIPPER;
		end
...
begin
	...
	begin
		if (present_state == STATE_2_ADD_MISSING_PART)
		...
		if (present_state == STATE_1_RUN_CLIPPER)
			stream_out_valid		<= pass_inner_frame & stream_in_valid;
		else if (present_state == STATE_2_ADD_MISSING_PART)
...
assign stream_in_ready		= 
	...
	(present_state == STATE_1_RUN_CLIPPER) ?
...
assign increment_counters	= 
	...
	(present_state == STATE_2_ADD_MISSING_PART) ?
		~pass_inner_frame | stream_out_ready | ~stream_out_valid :
		1'b0;
...
defparam	Clipper_Drop_Counters.WIDTH			= WIDTH,
			Clipper_Drop_Counters.HEIGHT		= HEIGHT,
