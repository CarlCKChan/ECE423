...
parameter WIDTH			= 640; // Final image width in pixels
parameter HEIGHT		= 480; // Final image height in lines
...
begin
	if (reset)
		width <= 'h0;
	else if (increment_counters & (width == (WIDTH - 1)))
		width <= 'h0;
...
begin
	...
	begin
		...
		else if (width == (WIDTH - RIGHT_OFFSET - 1))
...
begin
	...
	begin
		...
		else if (height == (HEIGHT - BOTTOM_OFFSET - 1))
...
assign end_of_outer_frame	=	(width == (WIDTH - 1)) & 
								(height == (HEIGHT - 1));

assign start_of_inner_frame	=	(width == LEFT_OFFSET) & 
								(height == TOP_OFFSET);
assign end_of_inner_frame	=	(width == (WIDTH - RIGHT_OFFSET - 1)) & 
								(height == (HEIGHT - BOTTOM_OFFSET - 1));
